...
const crearBalanceComprobacion = async (req, res) => {
  try {
    ...
    let totalDebitos = 0;
    let totalCreditos = 0;
    let saldoActualDebe = 0;
    let saldoActualHaber = 0;
    ...
    for (const element of libroDiario) {
      for (const detalle of element.detalle_libro_diarios) {
        ...
        if (tipo === "ACTIVOS") {
          ...
          cuentasActivos[detalle.str_detalle_libro_diario_codigo_cuenta].saldo_acreedora = cuentasActivos[detalle.str_detalle_libro_diario_codigo_cuenta].saldo_anterior_debito + cuentasActivos[detalle.str_detalle_libro_diario_codigo_cuenta].debe - cuentasActivos[detalle.str_detalle_libro_diario_codigo_cuenta].haber;
